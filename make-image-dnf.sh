...
usage() {
  cat <<EOF
Create a Docker base image using DNF.
...
dnf -y \
  --disablerepo='*' \
  $(for repo in "${repoids[@]}"; do echo "--enablerepo=$repo"; done) \
...
find "$installroot"/usr/share/locale -mindepth 1 -maxdepth 1 -not -name en -and -not -name en_US -and -not -name locale.alias -exec rm -rf {} +
